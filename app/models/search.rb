...
class Search < ApplicationRecord
  ...
  scope :top_searches, lambda {
                         select('query, COUNT(*) as count').group(:query).order(Arel.sql('COUNT(*) DESC')).limit(10)
                       } # query to get most repeated searches from different users
